    ...
    <div class="card-body">
        <form action="{{ url('/users/' . $user->id) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="number" step="0.01" class="form-control @error('balance') is-invalid @enderror" id="balance" name="balance" value="{{ old('balance', $user->balance) }}">
                @error('balance')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <select class="form-select @error('payment_status') is-invalid @enderror" id="payment_status" name="payment_status">
                    <option value="unpaid" {{ old('payment_status', $user->payment_status) == 'unpaid' ? 'selected' : '' }}>Unpaid</option>
            ...
            <div class="mb-3 form-check">
                <input type="checkbox" class="form-check-input @error('active_status') is-invalid @enderror" id="active_status" name="active_status" value="1" {{ old('active_status', $user->active_status) ? 'checked' : '' }}>
